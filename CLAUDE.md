...
## Session Handover Summary (อัปเดตล่าสุด: 22 ส.ค. 2026 — **V3.2.0-web-n-mobile** — web + mobile ใช้เลขเวอร์ชันเดียวกัน)
...
- **Release label ปัจจุบัน**: **V3.2.0-web-n-mobile** (tag `v3.2.0-web-n-mobile`) — web กับ mobile **เดินเลขเวอร์ชันร่วมกัน** (`3.2.0`) ป้าย `-web-n-mobile` เป็น**ชื่อรุ่นในเอกสารเท่านั้น** — **ห้ามใส่ลงฟิลด์ `version`** เพราะทั้ง pnpm และ `pub_semver` (force-update gate, ADR 0007) parse แบบ semver เคร่งครัด
- **เวอร์ชัน Flutter (logitrack-mobile)**: `pubspec.yaml` = **3.2.0+1** — โค้ด mobile ยังไม่เปลี่ยนจาก 3.1.0 (bump ตามเลขร่วม); ADR 0018 (ดาวน์โหลดรูปเอง) ยังไม่ implement
- **เวอร์ชัน Next.js (logitrack-web)**: **3.2.0** (`package.json` + `functions/package.json` + `shared-docs/package.json` + root `package.json` — bump พร้อมกันเสมอ) — Deploy ผ่าน Firebase Hosting + GitHub Actions (dev auto หลัง CI เขียว / prod manual `workflow_dispatch`)
