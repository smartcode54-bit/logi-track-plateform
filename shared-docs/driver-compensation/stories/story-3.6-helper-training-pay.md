...
- [x] **Mobile single-select alignment — i18n** (2026-06-24) — the single-select LOGIC was already correct (`_selectedHelperIds.clear()` + add-one + auto-pop); only the EN labels implied multiple. Singularised `assets/translations/en.json`: helper_section_title / helper_none_selected / helper_select / checkin_helpers. TH was already neutral. Chips render ≤1 item so display is fine.
  - Code-quality follow-up (NOT done — lives in the user's mobile WIP `check_in_page.dart`): de-duplicate the two near-identical helper pickers/write-sites (~lines 807/1473 and 992/1724).
...
- [x] **Recompute-on-change — finalized guard** (2026-06-24, branch feat/story-3.6-helper-followups) — `EditTripDetailsDialog` derives period+round from the task date and checks `payroll/{helperAuthId}_{period}_{round}`: APPROVED/PAID → field disabled + save blocked (use a Story 3.4 adjustment); DRAFT/PENDING_APPROVAL → shows a "regenerate to apply" hint. The DRAFT case is applied by re-running Generate Payroll (orchestrator idempotently overwrites DRAFT). Both the outgoing and incoming helper are checked on save. i18n task.helper.{locked,lockedSave,regenHint} EN+TH.
  - Still optional/nice-to-have: AUTO-triggering the DRAFT regenerate on edit (currently manual). Not built — full-batch regenerate from a dialog is the wrong shape; would need a single-driver recompute callable.
