...
/** Which date decides a customer's billing period + rate round (ADR 0027). */
type BillingDateBasis = "plan" | "delivered";

/** Per-customer billing inputs cached across a backfill run: rate rows + fuel rounds + date basis. */
...
function basisFromSnap(snap: admin.firestore.DocumentSnapshot): BillingDateBasis {
 ...
 */
async function resolveBillingDateBasis(
    db: admin.firestore.Firestore,
    entityId: string,
    custSnap?: admin.firestore.DocumentSnapshot
): Promise<BillingDateBasis> {
    const cs = custSnap ?? (await db.collection(COL_CUSTOMERS).doc(entityId).get());
    if (cs.exists) return basisFromSnap(cs);
 ...
 */
function resolvePlanBillingDateMs(basis: BillingDateBasis, task: Record<string, unknown>): number {
    if (basis !== "plan") return 0;
    const planMs = timestampLikeToMillis(task.date);
 ...
 */
async function stampBillingDateOnUnpriced(
    tripRef: admin.firestore.DocumentReference,
    billingDateTs: admin.firestore.Timestamp | null,
    customerId: string,
    tripId: string
): Promise<void> {
    if (!billingDateTs) return;
    try {
        ...
        logger.warn("[billingSnapshot] failed to stamp billingDate on unpriced trip", {
            tripId,
            error: String(e),
        });
    }
...
function mapFuelDoc(customerId: string, doc: admin.firestore.QueryDocumentSnapshot): FuelRateAdjustment {
    ...
    return {
        ...
        referenceFuelPriceThb:
            d.referenceFuelPriceThbPerLitre != null ? Number(d.referenceFuelPriceThbPerLitre) : undefined,
        voided: d.voided === true,
    };
}
...
async function tryWriteBillingSnapshotFromTripData(
    ...
    const customerId =
        taskInput.billingCustomerId?.trim() ||
    ...
    let billingDateBasis: BillingDateBasis;
    ...
    const billingDateMs = planBillingMs > 0 ? planBillingMs : timestampLikeToMillis(data.deliveredTimestamp);
    const billingDateTs: admin.firestore.Timestamp | null =
        billingDateMs > 0 ? admin.firestore.Timestamp.fromMillis(billingDateMs) : null;
...
interface SetTripJobCategoryRequest {
    tripId: string;
 ...
 */
export const setTripJobCategory = onCall<SetTripJobCategoryRequest, Promise<ComputeBillingResponse>>(
    {
        region: "asia-southeast1",
    ...
    async (request): Promise<ComputeBillingResponse> => {
        ...
        const targetCategory: "PRIMARY" | "SUPPLEMENTARY" =
            request.data?.jobCategory === "SUPPLEMENTARY" ? "SUPPLEMENTARY" : "PRIMARY";
        ...
        const data = tripSnap.data() as Record<string, unknown>;
        if (data.status !== "delivered") {
            throw new HttpsError("failed-precondition", "Trip is not delivered yet");
        }

        const taskId = typeof data.taskId === "string" ? data.taskId.trim() : "";
        if (!taskId) {
            throw new HttpsError("failed-precondition", "Trip missing taskId");
        }
        ...
        if (currentTaskCategory === targetCategory && data.jobCategory === targetCategory) {
            return {
                ...
                skipped: true,
                billingEstimateThb:
                    typeof data.billingEstimateThb === "number" ? (data.billingEstimateThb as number) : undefined,
            };
        }
        ...
        const customerId =
            ...
            "";
        if (!customerId) {
            throw new HttpsError("failed-precondition", "Task has no linked customer");
        }
        ...
        const noRateError = (): HttpsError => {
            const hubId = extractHubId(taskInput.sourceHub);
            const destination = normalizeDestinationCode(taskInput.destination);
            const vehicleClass = normalizeStoredCode(taskInput.truckType || "4WJ");
            logger.warn("[setTripJobCategory] no matching rate row for target category", {
                tripId,
                taskId,
                targetCategory,
                customerId,
            ...
            return new HttpsError(
                "failed-precondition",
                `No ${targetCategory} rate: ${hubId} → ${destination} (${vehicleClass})`
            );
        };

        // Compute FIRST; only build the write payload if a rate matches the target category.
        let tripUpdate: Record<string, unknown>;
        let newBillingEstimateThb: number;
        ...
        if (isMultiDelivery && deliveryStopsProgress.length >= 2) {
            ...
            if (stops.length < 2) {
                throw new HttpsError("failed-precondition", "Multi-delivery trip has < 2 delivered stops");
            }
            ...
            try {
                ...
                logger.warn("[setTripJobCategory] failed to load extra_stop service fee", {
                    customerId,
                    error: String(e),
                });
            }
            ...
            if (!multiComputed) throw noRateError();
            ...
            const computeForCategory = (cat: "PRIMARY" | "SUPPLEMENTARY") => {
                ...
                if (!c && codeToName && rawDestination) {
                    const altDest =
                        codeToName.get((resolvedDestination ?? rawDestination).trim()) ??
                        codeToName.get(rawDestination.trim());
                    if (altDest && altDest !== resolvedDestination) {
                        c = computeTripBillingFromParts(
                            tripParts,
    ...
    async (request): Promise<BackfillBillingResponse> => {
        ...
        if (filterCustomerId) {
            try {
                ...
                logger.warn("[billingSnapshot] backfill could not read customer basis", {
                    filterCustomerId,
                    error: String(e),
                });
            }
        ...
        const rateCache = new Map<string, CustomerBillingCache>();
        ...
        for (const doc of eligibleDocs) {
            ...
            if (typeof data.billingEstimateThb === "number") {
                const lock = locks.lockFor(
                    ...
                    timestampLikeToMillis(scanByPlanDate ? data.billingDate : data.deliveredTimestamp)
                );
                if (lock) {
                    blocked++;
