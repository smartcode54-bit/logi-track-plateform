 ...
 */
export function validateTripDocId(id: string): string | null {
    if (!id) return "Trip ID is required";
    if (id.includes("/")) return "Trip ID cannot contain '/'";
    if (id === "." || id === "..") return "Trip ID cannot be '.' or '..'";
    if (/^__.*__$/.test(id)) return "Trip ID cannot be wrapped in double underscores";
    // The limit is 1500 *bytes*: Thai trip labels cost 3 bytes per character.
    if (new TextEncoder().encode(id).length > 1500) return "Trip ID is too long";
    return null;
}
