...
export async function terminateAssignment(assignmentId: string, truckId: string, driverId: string) {
    try {
        await runTransaction(db, async (transaction) => {
            ...
            const driverDoc = await transaction.get(driverRef); // Check if driver exists
            ...
            if (driverDoc.exists()) {
                ...
                console.warn(`Driver ${driverId} not found during assignment termination. Skipping driver update.`);
            }
        });
        return true;
    } catch (error) {
